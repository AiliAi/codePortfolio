...
const Canvas = () => {
  ...
  const Sketch = p => {
    ...
    p.draw = () => {
      ...
      particles.push(new Particle(p, p.random(p.mouseX - newDistance, p.mouseX + newDistance), p.random(p.mouseY - newDistance, p.mouseY + newDistance)));
    };
  ...
  return (
    <div className="canvas-content" ref={processingRef} /> 
  );
};
